          ...
          child: Padding(
            ...
            child: reportAsync.when(
              ...
              data: (report) => report == null
                  ? _EmptyReportCard(year: _selectedYear, month: _selectedMonth, childId: childId)
  ...
  Widget build(BuildContext context) {
    return Column(
      children: [
        ...
        _AICommentCard(report: report),
        const SizedBox(height: 16),
        _ReportRadarCard(report: report),
        const SizedBox(height: 16),
        if (report.parentMessage != null)
          _ParentMessageCard(message: report.parentMessage!),
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Column(
        ...
        children: [
          Text('${report.year}年${report.month}月の成果',
              style: const TextStyle(color: Colors.white70, fontSize: 12)),
          const SizedBox(height: 12),
          Row(
            mainAxisAlignment: MainAxisAlignment.spaceAround,
            children: [
              _StatItem(label: '完了ストーリー', value: '${report.storiesCompleted}', unit: '個'),
              _StatItem(label: '学習時間', value: '${report.totalStudyMinutes}', unit: '分'),
              _StatItem(label: '獲得ポイント', value: '${report.totalPointsEarned}', unit: 'pt'),
            ],
          ),
        ],
      ),
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 12),
          if (report.highlightComment != null)
            _CommentBlock(emoji: '🌟', label: '今月のハイライト', text: report.highlightComment!),
          if (report.growthComment != null) ...[
            const SizedBox(height: 10),
            _CommentBlock(emoji: '📈', label: '成長ポイント', text: report.growthComment!),
          ],
          if (report.adviceComment != null) ...[
            const SizedBox(height: 10),
            _CommentBlock(emoji: '💡', label: '来月へのアドバイス', text: report.adviceComment!),
          ],
        ],
      ),
    );
...
class _ReportRadarCard extends StatelessWidget {
  final MonthlyReport report;
              ...
              RadarChartData(
                dataSets: [
                  RadarDataSet(
                    ...
                    entryRadius: 4,
                    dataEntries: [
                      RadarEntry(value: report.kindnessScore),
                      RadarEntry(value: report.honestyScore),
                      RadarEntry(value: report.responsibilityScore),
                      RadarEntry(value: report.courageScore),
                      RadarEntry(value: report.respectScore),
                      RadarEntry(value: report.cooperationScore),
                    ],
                  ),
                ],
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 12),
          Text(message, style: const TextStyle(fontSize: 14, color: _textPrimary, height: 1.6)),
        ],
      ),
    );
  }
